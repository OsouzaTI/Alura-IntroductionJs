...
function emitirCertificado(nome) {
  ...
  if(contadorDeChamadas < numeroChamadas && contadorDeEmissoesDeCertificado == 0) {
      ...
      if(contadorDeChamadas == 1) {
        mensagemTimeout = `Achou que ia ser f�cil?\nChame a fun��o emitirCertificado() de novo l� no seu c�digo!`
      }
      else if(contadorDeChamadas == 2) {
        mensagemTimeout = `Ainda n�o!\nChame emitirCertificado() mais uma vez no seu c�digo!`
      }
      else if(contadorDeChamadas < 100){
        mensagemTimeout = 
          `Boa! Mas voc� precisa chamar essa func�o 1 milh�o de vezes.\nVoc� j� chamou a fun��o ${contadorDeChamadas} ${
    ...
    if(contadorDeEmissoesDeCertificado > 0 && contadorDeChamadas > numeroChamadas) {      
      ...
      alert(
        `Loop Inifinito!\nVoc� chamou a fun��o emitirCertificado() ${totalChamadasLoopInfinito} vezes`
       )
      ...
      divMensagem.innerHTML = `
        Sucesso! 
        <br>
        Agora s� falta colocar o HTML do certificado na tela.
        <br>
        Esse HTML foi retornado pela fun��o emitirCertificado()
      `
      document.body.appendChild(divMensagem)
      alert(
        `Parab�ns ${nome}!\nVoc� concluiu a #quarentenaDev =D\nContinuar para o certificado?`
      )
